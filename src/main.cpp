...
void wireless(void *Param)
{
  ...
  for (;;)
  {
    ...
    publish(local_IP, "MQTT/TEST/IP");
#endif
    vTaskDelay(10000 / portTICK_PERIOD_MS);
  }
}
void pumpControl(void *Param)
{
  pinMode(20, OUTPUT);
  ...
  for (;;)
  {
    ...
    // Ring Blower Control Logic
    if (averagedomgl != NULL)
    {
      if (averagedomgl <= DO_LOWTHRES)
      {
        digitalWrite(20, HIGH);
      }
      else if (averagedomgl >= DO_LOWTHRES && averagedomgl <= DO_HIGHTHRES)
      {
        // DO Nothing
      }
      else if (averagedomgl >= DO_HIGHTHRES)
      {
        digitalWrite(20, LOW);
      }
    }
...
void setup()
{
  ...
  xTaskCreatePinnedToCore(pumpControl, "pumpControl", 5000, NULL, 1, NULL, 1);
  xTaskCreatePinnedToCore(pumpControl, "envSensors", 5000, NULL, 2, NULL, 1);
}
void loop()
{
  //
